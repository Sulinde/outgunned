...
- Item sheets use header where the item type is above the name, icon on the right if it has one.
- Replaces the popup menus for editing or deleting items on a sheet with buttons that show when the sheet is unlocked
- Replaces Futura with an OpenFont.  Dials back the use of Bold so it can be used for emphasis.
- Adds the same edit/delete buttons in gear, gun, condition, and ride grids.
